...
class JSONRPC(object):
	...
	def __interface__(self):
		return dict(
			...
			attrs=[x for x in dir(self) if
				   not callable(getattr(self, x)) and x not in ('__interface__', '__jsoncall__')],
			funcs=[x for x in dir(self) if callable(getattr(self, x)) and x not in (
				'__interface__', '__jsoncall__', '__jsoncall_one__', '__class__', 'make_result', 'make_error')]
		)
	...
	def make_error(self, json, code, msg, exc):
		return dict(
			id=json.get('id'),
			jsonrpc="2.0",
			error=dict(
	...
	def __jsoncall_one__(self, body):
		...
		try:
			...
			return self.make_error(json, 1, 'application exception', ex)
...
class Client(object):
	...
	def __docall__(self, req):
		...
		return self.__parse_response__(
			self.__session__.post(self.__endpoint__, data=simplejson.dumps(req), headers=heads).text)
...
def build_routes(api, base='/', sess=lambda: None):
	api_obj = API(api)

	@bottle.get('%sapi' % base)
	def api():
		return api_obj.__interface__()

	@bottle.get('%sapi/:objid' % base)
	def api(objid):
		global objects
		try:
			obj = int(objid)
			...
			obj = objid
		if obj in objects:
			return objects[obj].__interface__()
		else:
			return resolve(api_obj, obj).__interface__()

	@bottle.post('%sapi' % base)
	def api():
		return api_obj.__jsoncall__(sess)

	@bottle.post('%sapi/:objid' % base)
	def api(objid):
		global objects
		try:
			obj = int(objid)
			...
			obj = objid
		if obj in objects:
			return objects[obj].__jsoncall__(sess)
		else:
			return resolve(api_obj, obj).__jsoncall__(sess)


if __name__ == '__main__':
	build_routes(APIObj(), base='/base/')

	@route('/')
	def main():
		return SimpleTemplate(
			source='<html><head> <script src="//ajax.googleapis.com/ajax/libs/prototype/1.7.1.0/prototype.js"></script><script src="/js/api.js" type="text/javascript"></script></head></html>').render()
