...
public class ApiClient <ApiInt extends PythonMagic> extends RpcClient {
    ...
    public static <ApiClass extends PythonMagic> ApiClass API(Class<ApiClass> clazz) {
        ...
        return ((ApiClient<ApiClass>)global_client.get("api")).proxy(clazz);
    }
    public static GlobalsInterface Globals() {
        if (global_client == null)
            throw new RuntimeException("must have created at least one ApiClient first!");
        return ((ApiClient<GlobalsInterface>)global_client.get("globals")).proxy(GlobalsInterface.class);
    }
	public ApiClient(String base_endpoint, String endpoint) {
		super(base_endpoint, endpoint);
	}

    public ApiInt proxy(Class api_int) {
        return (ApiInt)Proxy.newProxyInstance(ClassLoader.getSystemClassLoader(), new Class[]{api_int}, new ProxyHandler<ApiInt>(this));
    }
